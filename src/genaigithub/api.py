...
CORS(
    ...
    origins=["http://localhost:4200"], # You'll need this, you cannot use * (wildcard domain) when using supports_credentials=True
)
...
def get_all_repositories():
    ...
    issues = github.search_issues(f'type:pr review:none is:open created:>{seven_days_ago} is:unmerged org:{organization}')
...
def process_pr():
    ...
    processor = RAGLLMProcessor(openai_api_key, pg_vector_store, memory_key=f"chat_history_{repo_name_for_memory}_{pr_number}", model_name="gpt-4")
